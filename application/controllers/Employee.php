...
class Employee extends Back_end {
	...
	{	
		if($this->session->userdata('user_details'))
		{
			...
			$data['roles_list']=$this->Employee_model->get_roles_list_for_group_leader();
			$this->load->view('employee/add',$data); 
			}
		}else{
	...
	public function addpost(){
		...
		redirect('employee/add');
             ...
             $save_data=array(
		       ...
		       if(count($save)>0){
			   ...
			   }else if($login_details['role_id']==4){
				 $admin=$this->Employee_model->get_admin_id($login_details['created_by']);
				...
				'updated_at'=>date('Y-m-d H:i:s'),
				'created_by'=>isset($login_details['e_id'])?$login_details['e_id']:'',
				'group_leader'=>isset($login_details['e_id'])?$login_details['e_id']:'',
				'manager_id'=>isset($admin['manager_id'])?$admin['manager_id']:'',
				'admin_id'=>isset($admin['admin_id'])?$admin['admin_id']:'',
				 );
		        $save=$this->Employee_model->save_employee_details($save_data);	
		       if(count($save)>0){
	...
	{	
		if($this->session->userdata('user_details'))
		{
		  ...
		  if($user_details['role_id']==1){
			...
			$data['employees_list']=$this->Employee_model->get_employees_list_group_leader($user_details['e_id']);
			//echo'<pre>';print_r($data);exit;
			$this->load->view('employee/group_leader_list',$data);
			}else if($data['userdetails']['role_id']==5){
	...
	public function editpost(){
		...
		redirect('employee/edit/'.base64_encode($post['e_id']));
			...
			$org_images=$employee_details['org_image'];
		       ...
		       $update_data=array(
			    'profile_pic'=>isset($images)?$images:'',
				'org_image'=>isset($org_images)?$org_images:'',
	            'name'=>isset($post['name'])?$post['name']:'',
	            'email'=>isset($post['email'])?$post['email']:'',
	            'age'=>isset($post['age'])?$post['age']:'',
	            'mobile_number'=>isset($post['mobile_number'])?$post['mobile_number']:'',
